...
%% full observer on the CCF form
fobsv_poles = [-20, -20, -20, -20];
L_t = acker(A_ccf', C_ccf', fobsv_poles);
L = L_t';
...
%% reduced order observer on the CCF form
robsv_poles = [-20, -20, -20];
...
L_t = acker(A22', A12', robsv_poles);
L = L_t';

A_robs = A22-L*A12;
B_robs = [B2 L A21]; % input is [u Yr x1]'
C_robs = eye(length(A_robs)); % since we want all the states
D_robs = zeros(size(B_robs));